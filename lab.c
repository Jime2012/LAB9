...
void __interrupt() isr (void){
    ...
    if(PIR1bits.ADIF){                      // Fue interrupción del ADC?
        if(ADCON0bits.CHS == 0){            // Verificamos sea AN0 el canal seleccionado
            ...
            CCPRD = map(ADRESH, IN_MIN, IN_MAX, OUT_MIN, OUT_MAX); // Valor de ancho de pulso
            CCPR2L = (uint8_t)(CCPRD>>2);    // Guardamos los 8 bits mas significativos en CPR1L
            CCP2CONbits.DC2B0 = CCPRD & 0b1;    // 0.5ms ancho de pulso / 25% ciclo de trabajo
            CCP2CONbits.DC2B1 = (CCPRD>>1) & 0b1;
        }
    ...
    if (INTCONbits.T0IF){ // INTERRUPCION DEL TMR0
            TMR0 = 240; //SE INGRESA NUEVAMENTE EL VALOR DEL TMR0
            INTCONbits.T0IF = 0; // SE LIMPIA LA BANDERA
           
            if (cont <= 255) // MULTIPLEXADO
                cont++;
            else 
                cont = 0;
...
void setup(void){
    ...
    ADCON0bits.CHS = 0b0010;    // Seleccionamos AN0
    ADCON1bits.ADFM = 0;        // Justificado a la izquierda
    ADCON0bits.ADON = 1;        // Habilitamos modulo ADC
    __delay_us(40);
    ...
    CCPR2L = 125>>2;
    CCP2CONbits.DC2B0 = 125 & 0b1;    // 0.5ms ancho de pulso / 25% ciclo de trabajo
    CCP2CONbits.DC2B1 = (125>>1) & 0b1;
